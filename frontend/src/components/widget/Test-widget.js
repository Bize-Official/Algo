import { useState, useEffect, useRef } from "react";
//import "./styles.css";
import styles from './TWidget.module.css';
const content1 = 
<><h2>Details</h2>
    <p>
    Vivamus volutpat ipsum ac ipsum feugiat, vel molestie elit
    vestibulum. Donec luctus commodo dictum. Aenean in turpis erat.
    Vestibulum imperdiet nibh. Ipsum ac ipsum feugiat, vel molestie.
    </p>
    <img src="data:image/jpeg;base64," alt="Example" />
    </>


const content2 = <><h2>Save</h2>
    <p>
    Vivamus volutpat ipsum ac ipsum feugiat, vel molestie elit
    vestibulum. Donec luctus commodo dictum. Aenean in turpis erat.
    Vestibulum imperdiet nibh. Ipsum ac ipsum feugiat, vel molestie.
    </p></>;

const content3 = <><h2>Message</h2>
<p>
Vivamus volutpat ipsum ac ipsum feugiat, vel molestie elit
vestibulum. Donec luctus commodo dictum. Aenean in turpis erat.
Vestibulum imperdiet nibh. Ipsum ac ipsum feugiat, vel molestie.
Vivamus volutpat ipsum ac ipsum feugiat, vel molestie elit
vestibulum. Donec luctus commodo dictum. Aenean in turpis erat.
Vestibulum imperdiet nibh. Ipsum ac ipsum feugiat, vel molestie.
Vivamus volutpat ipsum ac ipsum feugiat, vel molestie elit
vestibulum. Donec luctus commodo dictum. Aenean in turpis erat.
Vestibulum imperdiet nibh WHAT. Ipsum ac ipsum feugiat, vel molestie.
</p></>;

const content4 = <><h2>Profile</h2>
<p>
Vivamus volutpat ipsum ac ipsum feugiat, vel molestie elit
vestibulum. Donec luctus commodo dictum. Aenean in turpis erat.
Vestibulum imperdiet nibh. Ipsum ac ipsum feugiat, vel molestie.
</p></>;

const buttons = [
    { title: "Details", icon:<i className="bi bi-briefcase-fill h5"></i>, content: content1 },
    { title: "Save", icon:<i className="bi bi-bookmark h5"></i>, content: content2 },
    { title: "Message", icon:<i className="bi bi-chat-dots-fill h5"></i>, content: content3 },
    { title: "Profile", icon:<i className="bi bi-person-circle h5"></i>, content: content4 }
  ];
  export const TWidget = () => {
    const [activeBlock, setActiveBlock] = useState(0);
    const [contentHeight, setContentHeight] = useState(0);
    const contentRefs = useRef([]);
  
    useEffect(() => {
      // Update the height when the active block changes
      setContentHeight(contentRefs.current[activeBlock]?.clientHeight + 25 || 0);
    }, [activeBlock]);
  
    const toggleMenuBlock = (index) => {
      setActiveBlock(index);
    };
  
    return (
      <>
        <article className={styles.card}>
          <div className={styles.buttons}>
            {buttons.map((button, index) => (
              <button
                key={button.title}
                className={index === activeBlock ? `${styles.active}` : ""}
                onClick={() => toggleMenuBlock(index)}
              >
                {button.icon}
              </button>
            ))}
          </div>
          <div className={styles.wrapper} style={{ height: contentHeight }}>
            <div className={styles.content}>
              {buttons.map((button, index) => (
                <div
                  key={button.title}
                  ref={el => contentRefs.current[index] = el} // Assign ref to each block
                  className={styles.block}
                  style={{ display: index === activeBlock ? 'block' : 'none' }}
                >
                  {button.content}
                   

                </div>
              ))}
            </div>
          </div>
        </article>
      </>
    );
  };